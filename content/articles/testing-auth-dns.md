...
After your authoritative DNS server gets set up and running,
there are a couple of online testers to check the steps
and ensure that it is correct.

Do-It-Yourself EDNS Test
========================

Some commands to try for EDNS checkout are:
```bash
...
## This is an opportunistic test

One cannot test a zone for the ability to indicate a truncated answer (setting the TC flag in a response) unless
we know which query to send it that will result in an oversize UDP response. If you are testing your own zone, you kno
w what RRsets you have in it, so may be able to create a better test query. The reason our test uses DNSKEY and `+dnssec`
option is that there is a good chance that the query response for a DNSSEC-signed zone will exceed 512 bytes
...
The above expectations gets based in following preconditions:

* EDNS version 0.
* EDNS option 100 is not yet defined.
* EDNS flag defined is DNSSEC OK (DO).
* When EDNS version 1 gets defined, we expect to see:
* OPT record with version set to 0 or 1 (this is because EDNS compliance includes indicating which version the server supp
orts in a server response).
...
of DNS limits to 512-byte UDP packet at the most.
