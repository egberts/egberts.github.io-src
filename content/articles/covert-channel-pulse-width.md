...
Once I plotted these four time-delay groups, I converted it to a series of zeroes and ones.   And then rasterized it on a 80x25 ANSI-colored screen, low and behold, a visual pattern was discerned.  Entropy was too low to be encrypted.
...
Well, that's an easy one: a specially-designed "waterfall" router that introduces sporadic but randomized packet delay in between each packets.  Even though "waterfall" is symptomatic of a DMZ bastion gateway, it remains to be the best kind of router in which to introduce such defensive mechanism into.
...
About 2 decades later, this guy already started down this path after I've moved on; he calls it "[Steagonagraphic Packets](https://vimist.github.io/2019/01/30/Steganographic-Packets.html)".  But that's just the preliminary.
