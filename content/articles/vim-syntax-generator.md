...
DOT graph on LR parser gets created
...
and wieldy with 287 keywords.  No one should have to memorize all that.

Use the [NFTABLES](https://wiki.nftables.org/), a real-world example,
for this article.

NFTABLES is arguable the largest parser that I
have found from source-code search engines.
...
wc -l /tmp/x
```
The output provides a parser-size.  Actual value is going to be
...
Note: \* Bind9 `named` configuration parser
([`parser.c`](https://github.com/isc-projects/bind9/blob/main/lib/isccfg/parser.c))
is manually extracted and then used as
...
but numerous edges that interconnects them together.  (source: my compiler class).
...
A bit about me.  I've created large Vim syntax files; DNS zone files,
Bind9 named config, and a some in-house  ones.
...
repetitive methods used toward the creation of these large Vim syntaxes.
And after staring at a starry nights, an idea occurred to me:
...
GV-formatted files (was a DOT-format but renamed
...
its results on search engines is better than "GV".

BASIC LR PARSER
---------------
Since `bison` is a parser, the reader should familiarize
with basic LR parser concept in general, noteably
...
The following LR parser terms found in this article:
...
Reduce Transition
-----------------
Reduce transition denotes a special node id that has a letter "R"
...
corresponds to a nonterminal symbol).

GV Format option
----------------
Expanding from the online tutorials having `bison`-generated GV examples,
a real example is 
the [NFTABLES](https://wiki.nftables.org/) command line (CLI) parser for
a Linux firewall.
The first lines of Bison-generated `nftables.gv` (generated
from
[`parser_bison.y`](http://git.netfilter.org/nftables/tree/src/parser_bison.y)
) gets created by executing:

```bash
cd  nftables/src
bison --graph=nftables.gv parser_bison.y
```
and the first lines of `nftables.gv` content looks like:
```dot
...
------------------------
Python-based [graphviz]() DOT translation tool got selected here
...
I've evaluated NetworkX, pydot, and a some others from Github and
...
Node represents as a single ID number (not using '->' symbol).
...
Edge id identifies by a pair of node IDs.
...
* keyword, great for OR-logic of putting tokens together
* match
* region
...
used to ensure that these patterns are not used elsewhere
...
Mind-Meld
=========
Vim syntax and Bison are two disparate sets of terminologies.
...
```
One could call it a night and get done with the creation of
