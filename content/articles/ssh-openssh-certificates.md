...
Notes:
    ...
    Delete any existing known_host entries for “example.host.net”
...
Notes:
    ...
    It’s strongly suggested you not only use a passphrase, but also use a strong one. Anyone who signs with this key will be able to allow access to any user on all of your servers on your network.
...
Notes:
    ...
    This blocks the public key or certificate from being used system-wide
...
Notes:
    ...
    The wildcard prevents the host-key from being used by any host
...
Notes:
    ...
    Expirations work equally on user and server certificates.
...
As a result, some environments may be hard, if not impossible, to enforce only certificate based authentication. CA Key Management
CA Key Security
